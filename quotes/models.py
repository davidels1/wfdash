...
class QuoteRequest(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='quotes')  # Link to User model
    ...
    # voice_note = models.FileField(upload_to='quote_voice_notes/%Y/%m/', blank=True, null=True) # Removed voice_note
...
class VoiceNote(models.Model):
    quote_request = models.ForeignKey('QuoteRequest', related_name='voice_notes', on_delete=models.CASCADE)
    audio_file = models.FileField(upload_to='quote_voice_notes/%Y/%m/')
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"Voice Note for Quote {self.quote_request.quote_number}"
    ...
    def save_audio(self, base64_data):
        try:
            ...
            self.audio_file.save(file_name, ContentFile(file_data), save=False)
