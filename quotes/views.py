...
from .utils import generate_quote_qr_code
...
with transaction.atomic():
    if not Suppliers.objects.filter(suppliername="NOTE").exists():
        Suppliers.objects.create(
            suppliername="NOTE", number="N/A", contact="N/A", email="N/A"
        )


# Add this function near the top of your file, after imports
def is_note_item(item):
    """Check if an item is a note item (supplier name is 'NOTE')"""
    return item.supplier and item.supplier.suppliername == "NOTE"
...
def quote_list(request):
    ...
    is_invoicing = request.user.groups.filter(
        name="INVOICING"
    ).exists()  # Add this line

    # Get base queryset with all related fields to avoid N+1 queries
    if is_admin or is_quoter or is_buyer or is_invoicing:  # Add is_invoicing here
        quotes = (
            QuoteRequest.objects.select_related("customer", "assigned_to", "rep")
    ...
    # Include is_invoicing in the context
    context = {
        ...
        "is_rep": is_rep,
        "is_buyer": is_buyer,
        "is_invoicing": is_invoicing,  # Add this line
    }
...
def quote_detail(request, pk):
    ...
    if request.method == "POST":
        ...
        elif "approve" in request.POST and request.user.is_staff:
            # Handle quote approval
            quote.status = "approved"  # Change to the correct status in your system
            quote.save()
            messages.success(request, f"Quote #{quote.quote_number} has been approved.")
            return redirect("quotes:quote_detail", pk=quote.pk)
        elif "reject" in request.POST and request.user.is_staff:
            # Handle quote rejection
            quote.status = "rejected"  # Change to the correct status in your system
            quote.save()
    ...
    # Generate QR code
    qr_code = generate_quote_qr_code(request, quote.id)

    # Calculate overall markup for admin users
    overall_markup = None
    if request.user.is_staff:
        ...
        for item in items:
            if item.cost_price and item.selling_price and item.quantity:
                total_cost += item.cost_price * item.quantity
                total_selling += item.selling_price * item.quantity

        if total_cost > 0:
            overall_markup = ((total_selling - total_cost) / total_cost) * 100
            overall_markup = round(overall_markup, 2)
    ...
    return render(
        ...
        "quotes/quote_detail.html",
        {
            ...
            "overall_markup": overall_markup,  # Add this line
        },
    )
...
def save_quote_item(request, item_id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            note_supplier = supplier.suppliername == "NOTE"
        except Suppliers.DoesNotExist:
            pass

        # Only validate price fields if it's not a note item
        if not note_supplier:
        ...
        # Continue with existing code to save the item...
        try:
            # Process text fields
            ...
            try:
                ...
                item.selling_price = float(
                    request.POST.get(f"selling_price_{item_id}", 0)
                )
            except ValueError:
                pass
            ...
            return JsonResponse(
                {"status": "success", "message": "Item saved successfully"}
            )
        except Exception as e:
...
def generate_quote_pdf(request, quote_id):
    try:
        ...
        for item in items:
            # If supplier is "NOTE", force prices to zero regardless of what was entered
            if is_note_item(item):
                display_price = 0
                display_amount = 0
            else:
                display_price = item.selling_price
                display_amount = item.quantity * display_price
                total += display_amount  # Only add to total if not a note
            ...
            table_data.append(
                [
                    ...
                    str(item.quantity),
                    (
                        "" if is_note_item(item) else f"R {display_price:,.2f}"
                    ),  # Hide price for notes
                    (
                        "" if is_note_item(item) else f"R {display_amount:,.2f}"
...
def archived_quotes(request):
    ...
    # Add permission checks like in quote_list
    is_admin = (
        request.user.is_superuser or request.user.groups.filter(name="ADMIN").exists()
    )
    is_quoter = request.user.groups.filter(name="QUOTERS").exists()
    is_buyer = request.user.groups.filter(name="BUYER").exists()
    is_invoicing = request.user.groups.filter(name="INVOICING").exists()

    # Base queryset for archived quotes
    base_query = QuoteRequest.objects.filter(
    ...
    # Filter based on permissions
    if is_admin or is_quoter or is_buyer or is_invoicing:
        quotes_list = (
            base_query.select_related("customer", "rep")
            .prefetch_related("items")
            .order_by("-pdf_generated_at")
        )
    else:
        # Regular users only see their own quotes
        quotes_list = (
            base_query.filter(rep=request.user)
...
def update_item_pricing(request, item_id):
    ...
    try:
        ...
        return JsonResponse(
            {"status": "error", "message": f"Invalid number format: {str(e)}"},
            status=400,
        )
    except Exception as e:
        return JsonResponse(
            {"status": "error", "message": f"Error updating item: {str(e)}"}, status=500
        )
