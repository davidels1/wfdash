...
def quote_list(request):
    ...
    if is_admin or is_quoter or is_buyer:
        quotes = QuoteRequest.objects.all().select_related(
            'customer', 'rep'
    ...
    quotes_by_status = {}
    status_order = ['new', 'in_progress', 'complete', 'cancelled']  # Define status order
    ...
    template = 'quotes/quote_list_mobile.html' if is_mobile(request) else 'quotes/quote_list.html'
    return render(request, template, context)

@login_required
def quote_create(request):
    if request.method == 'POST':
        form = QuoteRequestForm(request.POST, request.FILES) # ADDED request.FILES
        if form.is_valid():
            try:
                # Create quote
                ...
                voice_note_data = request.POST.get('voice_note_data')
                if voice_note_data:
                    try:
                        voice_note = VoiceNote(quote_request=quote)
                        voice_note.save_audio(voice_note_data)
                        voice_note.save()
                        messages.success(request, "Voice note recorded and saved.")
                    except Exception as e:
                        messages.error(request, f"Error saving voice note: {e}")
...
def quote_edit(request, pk):
    ...
    if request.method == 'POST':
        form = QuoteRequestForm(request.POST, request.FILES, instance=quote) # ADDED request.FILES
        ...
        if form.is_valid() and formset.is_valid():
            ...
            if voice_note_data:
                try:
                    ...
                    voice_note.save_audio(voice_note_data)
                    voice_note.save()
                    messages.success(request, "Voice note updated.")
                except Exception as e:
                    messages.error(request, f"Error saving voice note: {e}")
