...
def quote_list(request):
    ...
    quotes = QuoteRequest.objects.all().select_related('customer', 'rep')
    
    if search_query:
        quotes = quotes.filter(
            Q(quote_number__icontains=search_query) |           # Quote number
            Q(customer__company__icontains=search_query) |      # Customer name
            Q(rep__first_name__icontains=search_query) |       # Rep first name
            Q(rep__last_name__icontains=search_query) |        # Rep last name
            Q(items__description__icontains=search_query) |     # Item descriptions
            Q(items__selling_price__icontains=search_query) |   # Selling price
            Q(items__cost_price__icontains=search_query) |      # Cost price
            Q(notes__icontains=search_query) |                 # Notes
            Q(status__icontains=search_query) |                # Status
            Q(created_at__date__year__icontains=search_query) |   # Year
            Q(created_at__date__month__icontains=search_query) |  # Month
            Q(created_at__date__day__icontains=search_query)      # Day
        ).distinct()
