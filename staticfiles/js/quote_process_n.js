...
function initQuoteProcessPage() {
    ...
    // Initialize form handlers (save, generate quote) - BUT WITHOUT ADDING GENERATE QUOTE BUTTON HANDLERS
    initFormHandlers();
    
    // Add this line to initialize the pricing modal
    initPricingModal();
    
    // Initialize quote generation - THIS SHOULD BE LAST TO OVERRIDE ANY PREVIOUS HANDLERS
    initQuoteGeneration();
...
// Revised save all items function - NO defaults, just validation and saving
async function saveAllItems() {
    console.log("Saving all items before generating quote...");
    
    // Get ALL items, not just unsaved ones
    ...
    console.log(`Found ${allItems.length} items to save`);
    
    // First validate again to ensure nothing is missing
    if (!validateAllItems()) {
        return false;
    }
    
    // If validation passes, save each item exactly as-is, no defaults
...
// Enhanced validation function - checks ALL items and reports ALL missing fields
function validateAllItems() {
    console.log("Validating all items before quote generation...");
    
    const requiredFields = [
        { selector: '[name^="quote_reference_"]', name: 'quote_reference', label: 'Quote Description' },
        { selector: '[name^="quantity_"]', name: 'quantity', label: 'Quantity' },
        { selector: '.supplier-select', name: 'supplier', label: 'Supplier' },
        { selector: '[name^="cost_price_"]', name: 'cost_price', label: 'Cost Price' },
        { selector: '[name^="markup_"]', name: 'markup', label: 'Markup' }
    ];
    ...
    // Check every item, regardless of whether it's already saved
    itemRows.forEach(row => {
        const itemId = row.getAttribute('data-item-id');
        const itemNumber = row.querySelector('.item-number')?.textContent.trim() || `Item #${itemId}`;
        const missingFields = [];
        
        // Check each required field for this item
        requiredFields.forEach(field => {
            const selector = field.selector.replace('_]', `_${itemId}]`);
    ...
    if (invalidItems.length > 0) {
        ...
        toastr.error(errorMessage, 'Validation Error', {
            closeButton: true,
            timeOut: 0,  // Don't auto-hide this error
            extendedTimeOut: 0,
            progressBar: false,
            enableHtml: true,
            newestOnTop: true,
...
// Replace the existing initQuoteGeneration function with this improved version
function initQuoteGeneration() {
    ...
    document.querySelectorAll('.generate-quote-btn').forEach(button => {
        button.addEventListener('click', async function(e) {
            ...
            this.innerHTML = '<i class="feather icon-check-circle fa-spin"></i> Validating...';
            this.disabled = true;
            
            try {
                // 1. First validate all items - this will show errors if any fields are missing
                ...
                // 2. Show saving indicator when validation passes
                this.innerHTML = '<i class="feather icon-refresh-cw fa-spin"></i> Saving all items...';
                
                // 3. Force click all save buttons individually rather than using saveAllItems
                const saveButtons = document.querySelectorAll('.save-item-btn');
                console.log(`Clicking ${saveButtons.length} save buttons...`);
                
                for (const saveBtn of saveButtons) {
                    const itemId = saveBtn.getAttribute('data-item-id');
                    console.log(`Attempting to save item ${itemId}...`);
                    saveBtn.click();
                    // Add a small delay between clicks to avoid overwhelming the server
                    await new Promise(resolve => setTimeout(resolve, 500));
                }
                
                // 4. Wait a moment for all saves to complete
                await new Promise(resolve => setTimeout(resolve, 1000));
                
                // 5. Update button text and proceed
                this.innerHTML = '<i class="feather icon-file-text fa-spin"></i> Generating...';
                console.log("All items saved, generating quote...");
    ...
    console.log("Quote generation buttons initialized");
}
