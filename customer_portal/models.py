...
class CustomerSubmission(models.Model):
    """Track anonymous submissions to associate with customer later if needed"""
    SUBMISSION_TYPES = (
        ('quote', 'Quote Request'),
        ('order', 'Order')
    )
    ...
    quote = models.ForeignKey('quotes.QuoteRequest', on_delete=models.SET_NULL, 
                              null=True, blank=True, related_name='customer_submission')
